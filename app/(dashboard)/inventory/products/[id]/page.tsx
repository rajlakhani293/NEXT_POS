...
import { ArrowLeft, CheckIcon, ChevronDownIcon, WandSparkles } from "lucide-react";
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-32">
        <DropdownMenuItem
          ...
          className={cn(isInclusive && "bg-accent font-semibold text-accent-foreground")}
        >
          {isInclusive ? <CheckIcon className="size-4" /> : <span className="size-4" />}
          with Tax
        </DropdownMenuItem>
        <DropdownMenuItem
...
function buildProductFormData(values: ProductFormValues, isEdit: boolean) {
  ...
  formData.append("track_stock", String(values.product_type === "stock" && Boolean(values.track_stock)));
  formData.append("allow_decimal_qty", String(Boolean(values.allow_decimal_qty)));
  formData.append("expiry_tracking_enabled", String(values.product_type === "stock" && Boolean(values.expiry_tracking_enabled)));
...
export default function ProductFormPage() {
  ...
  const validate = () => {
    ...
    if (!formData.selling_price) nextErrors.selling_price = "Selling price is required";
  ...
  const taxOptions = toOption(taxGroups.data?.data);
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-white">
      ...
      <div ref={contentRef} className="min-h-0 flex-1 overflow-y-auto">
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          <div className="flex flex-col gap-5 xl:flex-row px-4 pt-4">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-1">
                    <UniFieldInput
                      ...
                      addonAfter={
                        <TaxModeDropdown
                          ...
                          onChange={(value) => updateField("is_tax_inclusive", value)}
                        />
                      }
                    ...
                    <p className="text-xs font-semibold text-gray-500">
                      {formData.is_tax_inclusive ? "Inclusive of Taxes" : "Exclusive of Taxes"}
                    </p>
                  </div>
            ...
            <aside className="w-full space-y-5 xl:w-[30%]">
              ...
              <section className="rounded-lg border border-gray-200 bg-white p-4">
                <h2 className="mb-4 text-base font-semibold text-gray-900">Preferences</h2>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={name} className="flex items-center justify-between rounded-md border p-3">
                      ...
                      <Switch
                        checked={Boolean(formData[name as keyof ProductFormValues])}
                        disabled={!isStockProduct && name !== "allow_decimal_qty"}
                        onCheckedChange={(checked) =>
                          updateField(name as keyof ProductFormValues, checked)
                        }
                      />
                    </div>
                  ))}
          ...
          >
            ...
            >
              ...
              <Button type="submit" disabled={isSubmitting} className="min-w-28 bg-black text-white hover:bg-black/90">
                {isSubmitting ? (
